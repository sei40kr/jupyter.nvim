...
describe("jupyter.init", function()
	...
	describe("user autocommands", function()
		it("fires Init Pre/Post and KernelReady on start_kernel", function()
			local bufnr = helpers.scratch_buf({ "# %%", "x = 1" })
			vim.api.nvim_set_current_buf(bufnr)
			local events, group =
				helpers.capture_user_autocmds({ "JupyterInitPre", "JupyterInitPost", "JupyterKernelReady" })
			finally(function()
			...
			assert.equals("JupyterInitPre", events[1].pattern)
			assert.equals(bufnr, events[1].data.bufnr)
			assert.is_nil(events[1].data.kernel_id)

			assert.equals("JupyterInitPost", events[2].pattern)
			assert.equals(bufnr, events[2].data.bufnr)
			assert.equals("test-kernel", events[2].data.kernel_id)

			assert.equals("JupyterKernelReady", events[3].pattern)
			assert.equals(bufnr, events[3].data.bufnr)
			assert.equals("test-kernel", events[3].data.kernel_id)
		end)

		it("does not fire init events when a live kernel is already attached", function()
			local bufnr = helpers.scratch_buf({ "# %%", "x = 1" })
			vim.api.nvim_set_current_buf(bufnr)
			with_notify(function()
				jupyter.setup({})
		...
		it("fires Deinit Pre/Post on stop_kernel", function()
			local bufnr = helpers.scratch_buf({ "# %%", "x = 1" })
			vim.api.nvim_set_current_buf(bufnr)
			with_notify(function()
				jupyter.setup({})
			...
			local events, group = helpers.capture_user_autocmds({ "JupyterDeinitPre", "JupyterDeinitPost" })
			finally(function()
				vim.api.nvim_del_augroup_by_id(group)
			end)
			...
			assert.equals("JupyterDeinitPre", events[1].pattern)
			assert.equals(bufnr, events[1].data.bufnr)
			assert.equals("test-kernel", events[1].data.kernel_id)

			assert.equals("JupyterDeinitPost", events[2].pattern)
			assert.equals(bufnr, events[2].data.bufnr)
			assert.is_nil(events[2].data.kernel_id)
		end)

		it("does not fire Deinit events when no kernel is attached", function()
			local bufnr = helpers.scratch_buf({})
			vim.api.nvim_set_current_buf(bufnr)
			local events, group = helpers.capture_user_autocmds({ "JupyterDeinitPre", "JupyterDeinitPost" })
			finally(function()
		...
		it("fires KernelReady on restart_kernel", function()
			local bufnr = helpers.scratch_buf({ "# %%", "x = 1" })
			vim.api.nvim_set_current_buf(bufnr)
			with_notify(function()
				jupyter.setup({})
			...
			local events, group = helpers.capture_user_autocmds({ "JupyterKernelReady" })
			finally(function()
				vim.api.nvim_del_augroup_by_id(group)
			end)
			...
			assert.equals("JupyterKernelReady", events[1].pattern)
			assert.equals(bufnr, events[1].data.bufnr)
			assert.equals("test-kernel", events[1].data.kernel_id)
		end)
	end)
